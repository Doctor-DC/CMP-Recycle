...
SECRET_KEY = 'njtxfy0#u6nqf+c=dx2x5v@=4jb!11l*53bx5amo%&23b%^(-r'
...
STATIC_URL = '/static/'
STATIC_ROOT= os.path.join(BASE_DIR, "D:\MyDownloads\Recycle\Recycle\AllStatic")
...
LOGGING = {
    ...
    'loggers': {
        # 类型 为 django 处理所有类型的日志， 默认调用,django.server,django.request是子类
        'django': {
            'handlers': ['default','info','error', 'console'],
            'level': 'INFO',  #  level ：logger<handler
            'propagate': False  # 是否向父类传递消息
        },
        # log 调用时需要当作参数传入
        'log': {
